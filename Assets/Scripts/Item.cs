...
public class Item
{
    ...
    public Dictionary<Acquisition, int> takePercentByAcquisition { get; } //채집확률 프로퍼티 추가
    ...
    public Item(int itemId, string itemName, string type, string acquisitions, string needMaterialTypes, string needMaterialCounts, string takeTimes, string takePercent, int isConsumable, int maxCount, float createTime, int preytype, string effect, string decription)
    {
        ...
        string[] tempTakeTimes = (takeTimes ?? "").Split(",");
        string[] tempTakePercents = (takePercent ?? "").Split(",");
        string[] tempAquisitions = (acquisitions ?? "").Split(",");

        Dictionary<Acquisition, int> tempTakeTimesByAcquisition = new Dictionary<Acquisition, int>();
        Dictionary<Acquisition, int> tempTakePercentsByAcquisition = new Dictionary<Acquisition, int>();
        AcquisitionList = new List<Acquisition>();

        for (int i = 0; i < tempAquisitions.Length; i++)
        {
            if (string.IsNullOrEmpty(tempAquisitions[i]))
                continue;
            ...
            // time 안전 처리 (없으면 0)
            int takeTime = 0;
            if (i < tempTakeTimes.Length && !string.IsNullOrEmpty(tempTakeTimes[i]))
                int.TryParse(tempTakeTimes[i], out takeTime);

            // percent 안전 처리 (없으면 100)
            int percent = 100;
            if (i < tempTakePercents.Length && !string.IsNullOrEmpty(tempTakePercents[i]))
                int.TryParse(tempTakePercents[i], out percent);

            AcquisitionList.Add(acquisition);

            // 중복되는 키가 들어오면 덮어쓰기
            tempTakeTimesByAcquisition[acquisition] = takeTime;
            tempTakePercentsByAcquisition[acquisition] = percent;
        }

        takeTimeByAcquisition = tempTakeTimesByAcquisition;
        takePercentByAcquisition = tempTakePercentsByAcquisition;
